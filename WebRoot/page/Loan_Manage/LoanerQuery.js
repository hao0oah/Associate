...
$(function() {
    ...
    $("#gopage").click(function(){
    	...
    	if(currPage>0 && currPage<talpage)
    		queryLoaner(currPage);
    });
});

// 分页查询贷款人信息
function queryLoaner(pagenum) {
	var data = {};
	var bean = {};
    bean.bssav_acc_no = $("#bssav_acc_no").val();
    bean.cust_id = $("#cust_id").val();
    bean.cert_no = $("#cert_no").val();
    bean.party_name = $("#party_name").val();
    data.bean = bean;
    data.currentPage = pagenum;
    
    $.ajax({
        type: "POST",
        url: "getLoanerByPage.do",
        data: JSON.stringify(data),
        dataType: "JSON",
        contentType : "application/json",
        success: queryLoanerCallBack
    });
}

// 分页查询贷款人信息反调函数
function queryLoanerCallBack(data) {
	//由JSON字符串转换为JSON对象
	//var obj = eval('(' + date + ')');
...
function detail(data){
	self.location="LoanerDetail.jsp?"+JSON.stringify(data);
}
